...
public class MarksheetModel {
	...
	public List getMeritList(int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"SELECT `ID`,`ROLL_NO`, `NAME`, `PHYSICS`, `CHEMISTRY`, `MATHS` , (PHYSICS + CHEMISTRY + MATHS) as total,((PHYSICS + CHEMISTRY + MATHS)/3) as persent from `ST_MARKSHEET`where (physics>33 AND chemistry>33 AND maths>33)order by total desc");
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setTotel(rs.getInt(7));
				bean.setPersent(rs.getInt(8));
				list.add(bean);
			}
			rs.close();
		} catch (Exception e) {
